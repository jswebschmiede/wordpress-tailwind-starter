...
namespace Wtso\Custom;

use Wtso\Api\Settings;
use Wtso\Api\Callbacks\SettingsCallback;
...
class Admin {
	...
	private function enqueue() {
		...
		$scripts = array(
			'script' => array(
				'jquery',
				'media_uploader',
				get_template_directory_uri() . '/assets/dist/js/admin.bundle.js',
			),
			'style'  => array(
		...
		$pages = array( 'toplevel_page_wtso' );
	...
	private function enqueue_faq( Settings $settings ) {
		...
		// Pages array to where enqueue scripts
		$pages = array( 'wtso_page_wtso_faq' );
	...
	private function settings() {
		...
		$args = array(
			array(
				'option_group' => 'wtso_options_group',
				'option_name'  => 'first_name',
				'callback'     => array( $this->callback, 'wtso_options_group' ),
			),
			array(
				'option_group' => 'wtso_options_group',
				'option_name'  => 'wtso_test2',
			),
		);
	...
	private function sections() {
		...
		$args = array(
			array(
				'id'       => 'wtso_admin_index',
				'title'    => 'Settings',
				'callback' => array( $this->callback, 'wtso_admin_index' ),
				'page'     => 'wtso',
			),
		);
